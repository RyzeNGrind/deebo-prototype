...
const activeExtendedConnections = new Map();
/**
 * Connect to an MCP server using either stdio or SSE transport
 */
export async function connectExtendedMcpServer(name, serverName, sessionId, repoPath) {
 ...
 */
async function createExtendedMcpConnection(name, serverName, sessionId, repoPath) {
    const rawConfig = JSON.parse(await readFile(join(DEEBO_ROOT, 'config', 'extended-mcp-servers.json'), 'utf-8'));
 ...
 */
async function createStdioConnection(client, config, repoPath) {
    if (!config.command || !config.args) {
        throw new Error('Local server requires command and args');
    }
    // Handle environment variable substitutions
    let command = config.command;
    let args = config.args.map(arg => arg.replace(/{repoPath}/g, repoPath));
 ...
 */
async function createSSEConnection(client, config) {
    if (!config.url) {
 ...
 */
export async function validateMcpServer(serverName, repoPath = '/tmp') {
    try {
        const client = await connectExtendedMcpServer(`validator-${serverName}`, serverName, 'validation', repoPath);
 ...
 */
export async function getExtendedMcpServers() {
    const rawConfig = JSON.parse(await readFile(join(DEEBO_ROOT, 'config', 'extended-mcp-servers.json'), 'utf-8'));
    return rawConfig;
}
/**
 ...
 */
export async function cleanupExtendedConnections() {
