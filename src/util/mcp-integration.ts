...
const allMcpConnections: Map<string, Client> = new Map();

/**
 * Enhanced function that connects to both original and extended MCP tools
 */
export async function connectAllMcpTools(
  agentName: string, 
  sessionId: string, 
  repoPath: string
): Promise<{
  ...
  extendedClients: Map<string, Client>;
}> {
  try {
    ...
    const extendedClients = new Map<string, Client>();
 ...
 */
export function parseExtendedToolCalls(responseText: string, gitClient: Client, filesystemClient: Client, extendedClients: Map<string, Client>) {
  const toolCalls = [...responseText.matchAll(/<use_mcp_tool>([\s\S]*?)<\/use_mcp_tool>/g)].map(match => match[1].trim());
  
  return toolCalls.map((tc: string) => {
    try {
      ...
      if (!server) throw new Error(`Unknown or unavailable server: ${serverName}`);
 ...
 */
export async function generateEnhancedToolPrompt(
  gitClient: Client, 
  filesystemClient: Client, 
  extendedClients: Map<string, Client>
): Promise<string> {
 ...
 */
export async function cleanupAllMcpConnections(): Promise<void> {
